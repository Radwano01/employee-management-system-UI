...
const Calendar = () => {
    ...
    useEffect(() => {
        ...
        fetchData();
    }, [email]); // Include email as a dependency for useEffect
    ...
    const getDaysStartingFromFirstDay = () => {
        const firstDayOfMonth = new Date(2024, month - 1, 1).getDay(); // Adjust month to start from 0
    ...
    const isDateInRange = (task, date) => {
        ...
        const currentDate = new Date(2024, month - 1, date); // Adjust month to start from 0
